...
final class Store: ObservableObject {
    ...
    func setWordCapture(_ on: Bool) {
        ...
        if on && !CGPreflightScreenCaptureAccess() {
            ...
            captureNote = Note(text: L("没有截图权限：请在“系统设置 > 隐私与安全性 > 屏幕与系统录音”中打开蝶笺，然后重新打开本应用", "Screen Recording permission is missing: turn on Papillon (蝶笺) in System Settings > Privacy & Security > Screen & System Audio Recording, then reopen the app"), isError: true)
        }
        wordCapture = on
    }
    ...
    func setSelectTranslate(_ on: Bool) {
        ...
        if on && !AXIsProcessTrustedWithOptions([kAXTrustedCheckOptionPrompt.takeUnretainedValue(): true] as CFDictionary) {
            selectNote = Note(text: L("没有辅助功能权限：请在“系统设置 > 隐私与安全性 > 辅助功能”中打开蝶笺，划词翻译才能读取选中的文字", "Accessibility permission is missing: turn on Papillon (蝶笺) in System Settings > Privacy & Security > Accessibility so Select to translate can read the selected text"), isError: true)
        }
        selectTranslate = on
    }
